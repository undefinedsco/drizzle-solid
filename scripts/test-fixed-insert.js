...
async function testFixedInsert() {
  ...
  try {
    ...
    // 2. 现在运行修复后的 drizzle-solid 示例
    console.log('\n🚀 运行修复后的 drizzle-solid 示例...');
    
    // 导入修复后的代码
    const { drizzle } = await import('../src/index.js');
    const { podTable, string, int, date } = await import('../src/index.js');
    
    // 定义表结构（与 03-basic-usage.ts 相同）
    ...
    if (afterResponse.ok) {
      ...
      if (hasTestTask && hasTestTitle && afterCount > beforeCount) {
        ...
        try {
          ...
          const testTaskFromDb = allTasks.find(task => task.id === testTaskId);
          if (testTaskFromDb) {
